...
>
> To provide feedback on the distribution spec, please leave a comment [on spec issue #71](https://github.com/Unity-Billal-mesloub/Spec-Development-Containers/issues).

## Repo and Template Structure

This repository contains a _collection_ of two Templates - `hello` and `color`. These Templates serve as simple template implementations which helps containerize the project. Similar to the [`devcontainers/templates`](https://github.com/Unity-Billal-mesloub/templates) repo, this repository has a `src` folder.  Each Template has its own sub-folder, containing at least a `devcontainer-template.json` and `.devcontainer/devcontainer.json`. 
...
>
> While any registry [implementing the OCI Distribution spec](https://github.com/Unity-Billal-mesloub/distribution-spec) can be used, this template will leverage GHCR (GitHub Container Registry) as the backing registry.
...
```
ghcr.io/Unity-Billal-mesloub/template-starter/color:latest
ghcr.io/Unity-Billal-mesloub/template-starter/hello:latest
```
...
'`Unity-Billal-mesloub/template-starter`' is known as the template collection namespace.
...
* Go to [github.com/Unity-Billal-mesloub/devcontainers.github.io](https://github.com/Unity-Billal-mesloub/devcontainers.github.io)
     * This is the GitHub repo backing the [containers.dev](https://containers.dev/) spec site
* Open a PR to modify the [collection-index.yml](https://github.com/Unity-Billal-mesloub/devcontainers.github.io/blob/gh-pages/_data/collection-index.yml) file

This index is from where [supporting tools](https://containers.dev/supporting) like [VS Code Dev Containers](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers) surface Templates for their Dev Container Creation Configuration UI.

### Testing Templates

This repo contains a GitHub Action [workflow](.github/workflows/test-pr.yaml) for testing the Templates. Similar to the [`Unity-Billal-mesloub/templates`](https://github.com/Unity-Billal-mesloub/templates) repo, this repository has a `test` folder.  Each Template has its own sub-folder, containing at least a `test.sh`.
